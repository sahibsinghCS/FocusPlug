...
4. **Anti-if-else structure** (grafted from P2): the `research_churn` adversarial archetype (heavy switching entirely inside the allowlist, no drift) and a CI baseline gate — `forecast:eval` exits nonzero unless the MLP beats a single-feature logistic baseline by ≥ 0.03 held-out ROC-AUC.
...
### 1.2 Architecture — `TinyMLP 18→12→1`, 241 params
Hand-rolled forward pass in pure TypeScript, no new deps (mirrors the hand-rolled desk head — part of the pitch):

```
x ∈ R^18 (normalized features, §1.3)
h = tanh(W1·x + b1)        W1: 12×18 (216) + b1: 12  → 228
z = w2·h + b2 (logit)      w2: 12 + b2: 1            → 13
risk_raw = σ(a·z + b)      Platt calibration (a,b fit on val) → 2 (stored, not counted in "241")
total trainable: 241 params ≈ 5 KB JSON
```
...
- Forward pass ≈ 450 multiply-adds; with 18 occlusion passes and the calibration it is still < 0.1 ms — microseconds in Node or a browser. tfjs stays where it is (Desk AI).
- Why not LR: kept as baseline + last-resort fallback (§11); the MLP must beat it by the CI gate or it doesn't ship. Why not a GRU: the windowed features encode time already; a hand-rolled BPTT is the field's biggest overnight failure mode for zero extra on-screen value.
...
- `hidden[12]`: tanh activations for the internals strip.
...
- `modelVersion`, `paramCount`: on-screen provenance.
...
scripts/forecast/
  ...
  train.ts           hand-rolled MLP + Adam → src/shared/forecast/weights.json (+ golden fixture)
...
Levers: `--sessions --seed`, per-archetype weights, hazard scale, dwell jitter, desk-noise σ, grey vocabulary size, sensor-dropout probability, webcam-off spans. Default: 300 sessions ≈ 540 k raw frames ≈ 130 k train frames — seconds to train at 241 params.
...
### 4.4 Trainer (`train.ts`)
Adam (lr 3e-3, β 0.9/0.999), batch 256, class-weighted BCE, L2 1e-4, ≤ 400 epochs, early stop on val PR-AUC (patience 40), val = 10 % of train **sessions**, seed 42; float64 training, float32 shipped. Platt `(a,b)` fit on val logits only. **Committed gradient-check test** (grafted from P1's rigor at MLP cost): finite-difference vs analytic backprop on a 4→3→1 toy, rel. err < 1e-4 — the test that proves the ML is real. Output `src/shared/forecast/weights.json` (shape in Contracts) plus `src/shared/forecast/fixtures/golden.json` (8 input vectors → logit + risk at 1e-6) consumed by a runtime test proving the shipped TS forward pass matches the trainer bit-for-bit.
...
- **CI gate** (grafted from P2, softened against the 3 a.m. brick): exit nonzero unless MLP ROC-AUC − single-feature-logistic ROC-AUC ≥ 0.03 on eval. `--gate=off` bypasses but stamps `"gate":{"enforced":false}` into the committed report — the claim can be skipped, never faked. eval.ts also asserts `weights.thresholds` matches the `DEFAULT_SETTINGS` forecast keys (operating-point parity guard).
- Target line, filled with real numbers and the provenance caveat printed with it: *"Held-out sessions: ROC-AUC 0.90+, AUC@lead≥20s 0.85+, ~85 % of drifts caught with median 12–18 s warning, < 2 false pre-arms/hour — vs ~0.65 for the best single-signal heuristic. Trained on simulated + locally-augmented archetypes per the provenance block."* Same-seed rerun ⇒ byte-identical report.
...
- **Hidden-layer strip**: 12 cells tinted by tanh activation, labeled "hidden units". Honest spectacle, 12 divs.
- **Receipt line**: "✔ called it 18 s early" / "✘ missed — no warning" / "◌ pre-arm stood down · unconfirmed".
- **Model card footer** (always visible; fed from imported `weights.json` + `eval-report.json`): `TinyMLP 18→12→1 · 241 params · 5 KB · on-device · 1 Hz · v ff-1 · AUC 0.91 / lead≥20s 0.86 held-out · data: synthetic + local-aug (Adaption: offline-fallback 403)`.
...
| `src/shared/forecast/weights.json`, `eval-report.json`, `fixtures/golden.json` | shared artifacts | committed model + metrics(+provenance) + parity vectors |
...
| `scripts/forecast/{lib,simulate,build-dataset,adaption,augment-local,train,eval}.ts`, `GAUNTLET.md` | scripts | §4 |
...
- **0:00** — "Everything runs on this laptop — the model is 241 parameters, right there in the footer." Start session in VS Code. ON_TASK, meter ~8 %, teal bars: *solid desk presence*, *on-task streak* holding risk down.
...
- **1:05–1:25** — Close on the model card: "Held-out: AUC 0.91, lead-restricted AUC 0.86 — it scores frames twenty-plus seconds out, so it isn't just recognizing the last moment. 85 % of drifts caught, median 14 s warning, under 2 false alarms an hour — and when it misses or cries wolf, it prints that too. The build fails if this model can't beat a one-feature baseline. Provenance says exactly what was synthetic and that the sponsor pipeline is one flag away."
...
## 11. Risks + cut lines
**Risks & mitigations**
- *Trivial proxy ("any switching/grey ⇒ drift")*: research_churn + grinder + steady_then_snap archetypes; CI baseline gate; ablation table; per-archetype slices published.
