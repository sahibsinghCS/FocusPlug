...
Save PNGs at the paths below (create `docs/screenshots/` when filming). Freeze the overlay with `#/?scene=distracted&countdown=8&freeze=1` only for stills, not the live take. For the log still, `#/log?scene=golden` seeds a full causal chain on mock IPC; the live take should use the real log after Discord dies. Broken images here are slots, not missing product. The README embeds none of them on purpose — the only image it carries is generated by `npm run demo:verify` — so add a README still only once its PNG actually exists.
