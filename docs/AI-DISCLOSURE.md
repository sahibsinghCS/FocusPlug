...
1. **In-product ML** — **four models trained in this repo** (the Focus Forecast risk head, the adaptive fuse, the custom desk head, the attention head) plus two reused pretrained backbones, plus **one model that can be fitted on the student's own machine and never leaves it** (the personal attention head). Every one of them is a policy input: kill / no-kill / how long the fuse burns / whether to nudge.
2. **Build-time assistants** — coding agents that wrote this repo, and the one labelling service we bought annotations from.

Do not list a tool you did not use. Do not disclaim a model this repo actually trained: all four are committed, reproducible, and have printed numbers. The fifth is different in kind and is disclosed as such — it does not exist until a student fits it, it is fitted from their own photographs on their own machine, and it does not ship in this repository. Bracketed `⟦ ⟧` lines are blanks for the submitting team.
...
| **Personal attention head — fitted on the student's machine, never shipped** | The shipped attention head's 3×16 output layer — **51 numbers** — refitted from that student's own corrections and anchored to the shipped layer. `<userData>/desk-corrections/personal-attention-head.json`; nothing about it is committed to this repository | Replaces the shipped output layer for that install only, and **only if a gate lets it**: both heads are scored on the same 286-image held-out eval and eleven gates run in order (`docs/CORRECTION-LOOP.md` §7). It cannot exist without 12 deliberate corrections and a second, separate button — recording a correction has no call path to any fitting code. Its inputs are the student's own webcam frames, which **never leave the machine**, are listed in Settings and are deletable in one action that also deletes the head and stops it running — the cached model is dropped in the same call, so the reading after the tap is the shipped head's |
...
| Correction loop | `src/main/desk/corrections/` + `src/shared/correction/` | A pause the student says was wrong resumes the clock and silences that pause kind for a cooldown, and stores the frames that caused it with their label. Two timescales on purpose: the behavioural half is immediate and touches no weights; the statistical half accumulates and is folded in only at an explicit, gated refit. `docs/CORRECTION-LOOP.md` |
| Data leaving the device | None | Frames stay local, the forecast is local, the fuse model is local, corrected frames and any personal head stay in `<userData>/desk-corrections/`, no telemetry |
...
- **A personal attention head is not a better model, and the UI must never say it is.** What the gate licenses is a *do-no-harm* claim: the anchors it scores on are third-person stock photographs, so "not beaten on 286 stock images" says nothing about first-person accuracy. The only first-person evidence in the whole gate is the last one — *it agrees with you on N more of your own held-out corrections than the shipped head did* — and N is between 1 and 60, from one person in one room. Quote that sentence, with the correction count in it, or quote nothing. `npm run gauntlet:corrections` shows the gate is real in both directions (1.0% of shuffled-label pools install, 95.5% of genuine ones do), but it is a simulation against synthetic students, exactly like `gauntlet:adapt` and `gauntlet:plan`. And say plainly what it collects: the student's own webcam photographs, stored locally, never uploaded, visible and deletable — a feature that quietly kept images would be worse than one that never asked.
