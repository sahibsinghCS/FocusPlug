...
- **smart-plug-core** (`cursor/smart-plug-core-2e44`, PR #15): Kasa/HTTP/mock drivers on the frozen `PlugController` seam, hard protect (never study PC), settings `plugs` default `[]`, Demo Kill cuts secondary plugs. Docs: `docs/SMART-PLUGS.md`. Independent critic **WIN** (round 1) and **WIN** (round 2 after rebase onto contracts-hw + policy-plugs `67e8bbb`).
