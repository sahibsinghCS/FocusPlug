...
/**
 * Configurable-depth MLP with hand-rolled forward/backward + Adam — the same
 * shape of code as `scripts/forecast/train.ts`, generalized over the axes the
 * mlp-tuned sweep needs: hidden width, a second hidden layer, activation,
 * dropout, L2, LR schedule, batch size and per-sample loss weights.
 *
 * A finite-difference gradient check on a toy net runs before any training
 * (same forward/backward code path), so the backprop is proved, not presumed.
 */
