/**
 * Hand-rolled histogram gradient-boosted decision trees (pure TypeScript, no
 * dependencies) — the classic strong tabular baseline, sized for on-device
 * inference: shallow depth, modest tree count, shrinkage, row/column
 * subsampling, L2 leaf regularization.
 *
 * Design notes that matter for the honesty story:
 * - Split candidates come from QUANTILE BIN EDGES fit on the training-internal
 *   fit rows only. A split learned as `bin ≤ b` is stored as the REAL feature
 *   threshold `edges[f][b]`; `binOf(x) ≤ b ⇔ x ≤ edges[f][b]` by construction,
 *   so binned training and real-valued inference agree exactly (asserted by
 *   the caller on a sample of rows).
 * - Logistic loss: g = w·(σ(F) − y), h = w·σ(1−σ); leaf = −G/(H+λ) · lr.
 * - Histogram subtraction: the smaller child is scanned, the larger child is
 *   the parent minus the smaller — the standard LightGBM trick, which is what
 *   keeps a full hyper-parameter sweep inside a couple of minutes in JS.
 * - Everything is seeded (`mulberry32`), so a rerun is byte-identical.
 */
