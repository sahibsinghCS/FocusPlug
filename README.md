...
### 6. The correction loop — *the model is wrong; the student is the one who knows*

The phone head is the weakest thing in this product and the honest numbers above say so: 50–69% phone recall, and "phone" on about 17% of non-phone photos. So when a pause it raised was wrong, the paused screen asks — **I was working** / **You were right** — and that one tap does two things on two very different timescales.

**Immediately, it obeys.** The clock resumes and pauses of that kind are suppressed for a stated cooldown (25 minutes for phone, 10 for away). The student is the authority in the moment; the model is not. The cooldown is *derived from the stored corrections* rather than kept as a timestamp, which is not a detail: the resume that arms it calls `startSession()`, which resets the nudge tracker, so a stored timestamp would have been wiped by the very tap that set it. It also survives a restart, and deleting a correction drops the cooldown it armed — the state and the evidence for the state are the same object.

**Later, and only when asked, it teaches.** The frames that caused the pause are stored on the machine with the student's label. That is **first-person data from their own camera, room and lighting, captured at exactly the moments the model is worst, labelled by the only person who knows the truth** — everything else this head learned from is third-person stock photography.

**One click never retrains the model.** Corrections accumulate; recording one writes JPEGs and a JSON record and has no call path to any fitting code at all (`no-retrain.test.ts` asserts that by spying on the write seam). The refit is a separate button, needs 12 independent corrections, refuses while a session is running, and fits **the output layer only — 51 numbers** — anchored to the shipped layer, which the file format makes structural: a personal head physically cannot contain the 1280→16 representation, and `attention-head.json` is never opened for writing by the app.

**And a worse head must not ship.** Both heads are scored on the *same held-out eval* the table above reports, through eleven named gates in order, and the personal one installs only if it is not beaten pooled, drops neither slice by more than 3 points, and newly agrees with the student on at least one more of their **own** held-out corrections. A refit that fails writes the report and deletes any personal head that was there. Both columns and the blocking gate are on screen. `npm run gauntlet:corrections` checks the gate in **both** directions against 200 synthetic students a side — it installs 1.0% of heads fitted on shuffled labels and 95.5% of heads fitted on signal that is really there, because a gate nobody checked in the *pass* direction is indistinguishable from `return false`.

**Privacy is the whole premise.** These are the student's own webcam photographs. They live in `<userData>/desk-corrections/`, are **never uploaded** — there is no network import in that directory — are listed with thumbnails in Settings behind *Reveal folder*, and *Delete all* removes the frames, the index and any head fitted from them in one action — and drops the cached model with them, so the very next reading is the shipped head's and not the deleted one's. The only way one leaves the machine is the student running `npm run corrections:export` themselves. Full design, every gate and every refusal: [docs/CORRECTION-LOOP.md](docs/CORRECTION-LOOP.md).
...
Typecheck and the vitest suite take about twenty seconds together on a four-core Linux box (`npm ci` depends on your npm cache). `npm run typecheck` starts with `npm run check:contracts`, which byte-compares every file a doc publishes as complete source against that file — `src/shared/types.ts` against the frozen Types fence in [docs/CONTRACTS.md](docs/CONTRACTS.md), `src/shared/plan/types.ts` and `src/shared/plan/constants.ts` against the frozen appendix in [docs/FOCUS-PLAN.md](docs/FOCUS-PLAN.md), and `src/shared/correction/types.ts` and `src/shared/correction/constants.ts` against the one in [docs/CORRECTION-LOOP.md](docs/CORRECTION-LOOP.md) — so the contract docs cannot drift from the code in silence.
