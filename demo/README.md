...
| `src/pipeline.ts` | The one loop. `TelemetryRing` → `extractFeatures` → the trained 24→36→1 MLP head (`mlp24-36-1`, 937 params) → `smoothRisk` → `stepEscalation`, with `stepPolicy` deciding and `effectiveFuseSec` as the only knob the forecast turns. Both modes step this. |
