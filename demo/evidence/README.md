...
| `demo-mid-rise-1280x800.png` | `?t=33&freeze=1` — risk 48 %, 24 attributions, hidden-layer strip, and `logit +2.86 → σ(a·z+b) a=2.09 b=−5.80 → risk 0.54` end to end inside the frame |
| `demo-nudge-1280x800.png` | `?t=38&freeze=1` — sustained above the nudge threshold: toast, zero enforcement |
| `demo-prearm-1280x800.png` | `?t=79&freeze=1` — pre-armed: fuse plate reads `FUSE 10s → 5s`, still no violation |
| `demo-kill-receipt-1280x800.png` | `?t=86&freeze=1` — the policy engine burning the shortened fuse, with the lead-time receipt |
| `demo-unlocked-1280x800.png` | `?t=92&freeze=1` — unlocked, sparkline showing nudge ▲ / pre-arm ◆ / drift ✖ |
...
Every scene that shows the panel is also measured, not just photographed: the calibration readout must render end to end (`scrollWidth ≤ clientWidth`) and lie wholly inside the 800 px frame, and no element anywhere on the page may be hiding part of its own text at 1280 px or 900 px. Those assertions exist because an earlier cut of this page shipped the readout ellipsed mid-`σ(a·z+b)` at every width and below the fold in the very still that shows risk climbing.
