...
export function toneDot(tone: Tone): string {
  ...
  if (tone === "amber") return "bg-fp-amber shadow-[0_0_8px_rgba(232,161,60,0.85)]";
  return "bg-[#4a4354]";
}
...
export function toneChip(tone: Tone): string {
  ...
  if (tone === "amber") return "border-fp-amber/40 bg-fp-amber/10 text-fp-amber";
...
export function toneCard(tone: Tone): string {
  ...
  if (tone === "amber") return "border-fp-amber/30";
