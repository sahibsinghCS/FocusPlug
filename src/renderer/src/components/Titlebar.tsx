...
export function Titlebar(props: { route: RouteId }): JSX.Element {
      ...
      <div className="flex min-w-0 flex-1 items-center gap-2">
        ...
        <div className="min-w-0">
          <p className="flex items-baseline gap-2 leading-none">
            ...
            {app.usingMock ? <span className="sr-only">Using mock IPC</span> : null}
          </p>
        </div>
        ...
        <p className="hidden min-w-0 sm:block">
          <span className="truncate text-[13px] font-medium text-fp-ink">{copy.title}</span>
          <span className="ml-2 hidden text-[11px] text-fp-faint lg:inline">{copy.kicker}</span>
        </p>
      </div>

      <StatusCluster />

      <time className="hidden shrink-0 px-1 font-mono text-[11px] text-fp-faint tabular lg:inline">
        {clock}
      </time>
    </header>
  );
